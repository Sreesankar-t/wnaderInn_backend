...
import Review from '../models/user/reviewModal.js'
...
// REVIEW & RATING CONTROLLERS START
...
const getReview = asyncHandler(async (req, res, next) => {
  const id = req.params.id
  try {
    const data = await Review.find({ hotelId: id })
    res.status(200).json(data)
  } catch (error) {
    next(error)
  }
...
const getSpcificRatingCount = asyncHandler(async (req, res) => {
  const count = req.query.count.split(',')
  const hotelId = req.params.id
  try {
    const list = await Promise.all(
      count.map(c => {
        return Review.countDocuments({hotelId:hotelId , rating: c })
      })
    )
...
const getTotalReview = asyncHandler(async (req, res, next) => {
  const hotelId = req.params.id
  try {
    const doc = await Review.countDocuments({hotelId:hotelId})
...
const getTotalRating = asyncHandler(async (req, res, next) => {
  const hotelId = req.params.id
  try {
    const pipeline = [
      {
    ...
    const total = await Review.aggregate(pipeline)
    if (total.length > 0) {
      const totalRating = total[0].totalRating
